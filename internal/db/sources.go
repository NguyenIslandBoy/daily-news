...
func GetSources(pool *pgxpool.Pool) ([]models.Source, error) {
	rows, err := pool.Query(context.Background(),
		`SELECT id, name, url, feed_url, type, category, active, last_scraped_at
		 FROM sources WHERE active = true ORDER BY name`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var s models.Source
		if err := rows.Scan(&s.ID, &s.Name, &s.URL, &s.FeedURL,
			&s.Type, &s.Category, &s.Active, &s.LastScrapedAt); err != nil {
			return nil, err
		}
		sources = append(sources, s)
	}
